...
#define SIO_RX_BUF_SIZE 128  /* must be power of 2 */
#endif
...
int SioOpen(const char *pPortName,   /* is ignored */
    ...
    ucsrb |= (1 << RXEN1) | (1 << RXCIE1) | (1 << TXEN1);
    ucsrb &= ~(1 << UDRIE1);
    UCSR1B = ucsrb;
...
ISR(USART1_RX_vect) {
    ...
    case eTxing:
        ...
        if (UDR1 == sTxBufferBuffered[sComm.txRxComparePos]) {
