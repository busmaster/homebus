...
*  Macros
*/
#define NUM_BUTTONS   250  /* 125 Schaltermodule mit je 2 Tastern */
#define TASTTIME 600
...
static bool sWcDimm = false;
static uint8_t busVarNr0 = 0;
static bool sMerk[NUM_BUTTONS];
...
void ApplicationInit(void) {
    ...
    if (BusVarRead(0, &aktiv, sizeof(aktiv), &result) != sizeof(aktiv)) {
        return;
    }	
...
void ApplicationStart(void) {	
	PwmSet(ePwm0,255);  // Julia Haupt
	PwmSet(ePwm1,255);  // Julia neben
	PwmSet(ePwm2,255);  // Fabian Haupt
	PwmSet(ePwm3,255);  // Fabian Tisch
	PwmSet(ePwm4,255);  // Tobias Haupt
	PwmSet(ePwm5,255);  // Tobias neben
...
void ApplicationReleased29_1(void) {
   ...
   GET_TIME_MS32(actualTime);
   if (((uint32_t)(actualTime - sSttime[29+(NUM_BUTTONS/2)].startTime)) <= TASTTIME) {
	  PwmIsOn(ePwm8, &pOn);
	  if (pOn) {
	     PwmOn(ePwm8,false);
	  } else {
	     PwmDelayedOff(ePwm8, 60UL * 10UL * 1000UL);
	  }	
   }  	
}
...
void ApplicationPressed33_0(void) {
   GET_TIME_MS32(sSttime[33].startTime);		
}
void ApplicationReleased33_0(void) {
   ...
   GET_TIME_MS32(actualTime);
   if (((uint32_t)(actualTime - sSttime[33].startTime)) <= TASTTIME) {
	  PwmIsOn(ePwm8, &pOn);
	  if (pOn) {
	     PwmOn(ePwm8,false);
	  } else {
...
void ApplicationPressed35_0(void) {
   GET_TIME_MS32(sSttime[35].startTime);	
}
void ApplicationReleased35_0(void) {
   uint32_t      actualTime;	
   GET_TIME_MS32(actualTime);

   if (((uint32_t)(actualTime - sSttime[35].startTime)) <= TASTTIME) {
	   PwmToggle(ePwm5);
   }	
}
void ApplicationPressed35_1(void) {
   GET_TIME_MS32(sSttime[35+(NUM_BUTTONS/2)].startTime);	
   sMerk[35+(NUM_BUTTONS/2)] = 1;
}
void ApplicationReleased35_1(void) {
   ...
   PwmSetState(ePwm4, ePwmNoFade); // Stoppe dimmen
...
void ApplicationPressed38_0(void) {
   GET_TIME_MS32(sSttime[38].startTime);	
}
void ApplicationReleased38_0(void) {
   uint32_t      actualTime;	
   GET_TIME_MS32(actualTime);

   if (((uint32_t)(actualTime - sSttime[38].startTime)) <= TASTTIME) {
	   PwmToggle(ePwm3);
   }	
}
void ApplicationPressed38_1(void) {
   GET_TIME_MS32(sSttime[38+(NUM_BUTTONS/2)].startTime);	
   sMerk[38+(NUM_BUTTONS/2)] = 1;
}
void ApplicationReleased38_1(void) {
   ...
   PwmSetState(ePwm2, ePwmNoFade); // Stoppe dimmen
/* Aktion in ApplicationCheck */	   
   }
}
...
void ApplicationPressed40_0(void) {
   GET_TIME_MS32(sSttime[40].startTime);	
}
void ApplicationReleased40_0(void) {
   ...
   if (((uint32_t)(actualTime - sSttime[40].startTime)) <= TASTTIME) {	
      PwmToggle(ePwm1);
   }   
}
void ApplicationPressed40_1(void) {
   GET_TIME_MS32(sSttime[40+(NUM_BUTTONS/2)].startTime);	
   sMerk[40+(NUM_BUTTONS/2)] = 1;
}
void ApplicationReleased40_1(void) {
   ...
   PwmSetState(ePwm0, ePwmNoFade); // Stoppe dimmen
/* Aktion in ApplicationCheck */	   
   }
}
...
void ApplicationPressed46_1(void) {
   GET_TIME_MS32(sSttime[46+(NUM_BUTTONS/2)].startTime);	
}
void ApplicationReleased46_1(void) {
   ...
   GET_TIME_MS32(actualTime);
   if (((uint32_t)(actualTime - sSttime[46+(NUM_BUTTONS/2)].startTime)) > TASTTIME) {
      if (sWcDimm) {
	     val = 0;
		 BusVarWrite(0, &val, sizeof(val), &result);
      } else {
	     val = 1;
		 BusVarWrite(0, &val, sizeof(val), &result);
      }
...
void ApplicationPressed50_0(void) {
	if (sWcDimm == false) {
		PwmDimmTo(ePwm7,255,1000);
	}
	PwmDimmTo(ePwm6,100,1000);
}
void ApplicationReleased50_0(void) {
	PwmDimmTo(ePwm6,0,1000);
	PwmDimmTo(ePwm7,0,1000);	
}
...
void ApplicationPressed55_0(void) {
   GET_TIME_MS32(sSttime[55].startTime);	
}
void ApplicationReleased55_0(void) {
   ...
   if (((uint32_t)(actualTime - sSttime[55].startTime)) <= TASTTIME) {
      PwmToggle(ePwm3);
   }	
}
void ApplicationPressed55_1(void) {
   GET_TIME_MS32(sSttime[55+(NUM_BUTTONS/2)].startTime);	
   sMerk[55+(NUM_BUTTONS/2)] = 1;
}
...
void ApplicationPressed56_0(void) {
   GET_TIME_MS32(sSttime[56].startTime);	
}
void ApplicationReleased56_0(void) {
   ...
   if (((uint32_t)(actualTime - sSttime[56].startTime)) <= TASTTIME) {
      PwmToggle(ePwm5);
   }	
}
void ApplicationPressed56_1(void) {
   GET_TIME_MS32(sSttime[56+(NUM_BUTTONS/2)].startTime);	
   sMerk[56+(NUM_BUTTONS/2)] = 1;
}
