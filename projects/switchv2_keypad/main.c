...
static void CheckEvent(void) {
    ...
    case eInit:
        ...
        pActVal->actualValue.keyb.keyCode = sKeyCur;
...
/* ADC values for keys (1k pullup at PC1):
3    0000   0000 .. 013f
6    0280   0140 .. 03bf
9    0500   03c0 .. 06df
bell 08c0   06e0 .. 0aff
...
static void ProcessBus(void) {
    ...
    if (ret == BUS_MSG_OK) {
        ...
        case eBusDevReqActualValue:
            ...
            sTxBusMsg.msg.devBus.x.devResp.actualValue.devType = eBusDevTypeKeyb;
            sTxBusMsg.msg.devBus.x.devResp.actualValue.actualValue.keyb.keyCode = sKeyCur;
            BusSend(&sTxBusMsg);
            break;
        case eBusDevRespActualValueEvent:
            pClient = sClient;
            for (i = 0; i < sNumClients; i++) {
                if ((pClient->address == spRxBusMsg->senderAddr) &&
                    (pClient->state == eWaitForConfirmation)) {
                    if (spRxBusMsg->msg.devBus.x.devResp.actualValueEvent.actualValue.keyb.keyCode == sKeyCur) {
