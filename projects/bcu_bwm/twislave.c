		...
		case TW_SR_STOP: //prüfe Daten 							// 0xA0 STOP empfangen
			...
			if(rxbuffer[1] == 0xF5) {
				if((rxbuffer[buffer_adr-2]==((rxcrc[buffer_crc-3]>>8)&0xFF)) && (rxbuffer[buffer_adr-1]==(rxcrc[buffer_crc-3]&0xFF))) {// Empfangen CRC prüfen
				   switch (rxbuffer[3]) {
					   case 0x04: if( (rxbuffer[2] == 0x00) && (rxbuffer[4] == 0x00) && (rxbuffer[5] == 0x00) && (rxbuffer[6] == 0x02)) {// Schiebeschalter empfangen
                                      switch_register = rxbuffer[7]+0x80; // Höchstes Bit für neue Daten setzen
								  }
								  break;
								  
					   case 0x05: if( (rxbuffer[2] == 0x00) && (rxbuffer[4] == 0x02) && (rxbuffer[5] == 0x00)) {// lux- oder time poti empfangen
					                  if( rxbuffer[6] == 0x03) { // time_poti
									      time_poti = (((uint16_t) rxbuffer[7])<<8) + rxbuffer[8]; // 0x0a - 0x780 sekunden
									  }
					                  else if( rxbuffer[6] == 0x04) { // lux_poti
									      lux_poti = rxbuffer[8];
									  }									  
								  }
								  break;	

					   case 0x06: if( (rxbuffer[2] == 0x00) && (rxbuffer[4] == 0x00) && (rxbuffer[5] == 0x00) && (rxbuffer[6] == 0x01) && (rxbuffer[7] == 0x00)) {// lux und bwm empfangen
					                  bwm_on_register = rxbuffer[9]+0x80; // Höchstes Bit für neue Daten setzen	
					                  lux_register =  rxbuffer[8]; // sowieso alle 2 sek neu
								  }
								  break;
...
void set_SWITCH(void){
   uint8_t i;
   uint8_t  const data[20] = {0xBE,0x3F,0xF5,0x00,0x0F,0x00,0x00,0x00,0x00,0x15,0x07,0xD0,0x08,0x00,0x00,0x00,0x00,0x00,0x00,0x00};	
   uint16_t crc = 0x1d0f;
...
void init_LED(void) {
   ...
   uint8_t i;
   uint8_t const data[9] = {0x00,0x3F,0xF0,0x00,0x04,0x50,0xBE,0x00,0x01};	
   uint16_t crc = 0x1d0f;
...
void set_SWITCH1(void){
   uint8_t i;
   uint8_t  const data[8] = {0xBE,0x3F,0xF5,0x00,0x03,0x01,0x00,0x03};	
   uint16_t crc = 0x1d0f;
...
void set_SWITCH2(void){
   uint8_t i;
   uint8_t  const data[8] = {0xBE,0x3F,0xF5,0x00,0x03,0x01,0x00,0x04};	
   uint16_t crc = 0x1d0f;
