...
int main(int argc, char *argv[]) {
    ...
    do {
        ...
        case OP_GET_ACTUAL_VALUE:
            ...
            if (ret) {
                ...
                case eBusDevTypePwm4:
                    ...
                    break;
                case eBusDevTypePwm16:
                    printf("\nstate: %04x\n", actVal.actualValue.pwm16.state);
                    printf("pwm: ");
                    for (i = 0; i < BUS_PWM16_PWM_SIZE_ACTUAL_VALUE; i++) {
                        printf("%02x ", actVal.actualValue.pwm16.pwm[i]);
                    }
                    printf("\n");
                    break;
                case eBusDevTypeSmIf:
        ...
        case OP_SET_VALUE_PWM4:
            ...
            break;
        case OP_SET_VALUE_PWM16:
            setVal.devType = eBusDevTypePwm16;
            memset(setVal.setValue.pwm16.pwm, 0, sizeof(setVal.setValue.pwm16.pwm));
            setVal.setValue.pwm16.set = 0;
            for (j = argi, k = 0; (j < argc) && (k < (int)sizeof(setVal.setValue.pwm16.pwm)); j++) {
                if ((j % 2) == 0) {
                    setVal.setValue.pwm16.set |= (uint32_t)((atoi(argv[j]) & 0x3) << (k * 2));
                } else {
                    setVal.setValue.pwm16.pwm[k] = (uint8_t)atoi(argv[j]);
                    k++;
                }
            }
			printf("%08x\n", setVal.setValue.pwm16.set);
            ret = ModuleSetValue(moduleAddr, &setVal);
            if (ret) {
                printf("OK\n");
            }
...
static bool ModuleGetActualValue(uint8_t address, TBusDevRespActualValue *pBuf) {
    ...
    if (responseOk) {
        ...
        case eBusDevTypePwm4:
            ...
            break;
        case eBusDevTypePwm16:
            pBuf->actualValue.pwm16.state = pBusMsg->msg.devBus.x.devResp.actualValue.actualValue.pwm16.state;
            memcpy(pBuf->actualValue.pwm16.pwm,
                   pBusMsg->msg.devBus.x.devResp.actualValue.actualValue.pwm16.pwm,
                   sizeof(pBuf->actualValue.pwm16.pwm));
            break;
        case eBusDevTypeSmIf:
...
static int GetOperation(int argc, char *argv[], int *pArgi) {
    ...
    *pArgi = 0;
    ...
    for (i = 1; (i < argc) && (operation == OP_INVALID); i++) {
        ...
        } else if (strcmp(argv[i], "-setvalpwm16") == 0) {
            if (argc > i) {
                *pArgi = i + 1;
                operation = OP_SET_VALUE_PWM16;
            } else {
                break;
            }
...
static void PrintUsage(void) {
    ...
    printf("-setvalpwm16 set0 pwm0 .. set15 pwm15: setX = command, pwmX = 16 bit value\n");
