...
typedef struct {
   UINT16 wordAddr;
   UINT16 data[BUS_FWU_PACKET_SIZE / 2];
...
typedef struct {
   UINT8 dirSwitch[BUS_DO31_NUM_SHADER];
   UINT8 onSwitch[BUS_DO31_NUM_SHADER];
} TBusDevInfoDo31;
...
typedef struct {        
   TBusDevType devType;
   UINT8 version[BUS_DEV_INFO_VERSION_LEN];
   union {
...
typedef struct {
   UINT8 digOut[BUS_DO31_DIGOUT_SIZE_SET];  /* je Ausgang 2 Bit:           */
                                            /*                     0*: keine Änderung */
                                            /*                     10: Ausgang auf 0  */
                                            /*                     11: Ausgang auf 1  */
   UINT8 shader[BUS_DO31_SHADER_SIZE_SET];      /* je Rollladen 2 Bit: 00: keine Act. */
                                            /*                     01: AUF        */
                                            /*                     10: AB         */
                                            /*                     11: STOP       */
} TBusDevSetStateDo31;
...
typedef struct {
   UINT8 digOut[BUS_DO31_DIGOUT_SIZE_GET];  /* je Ausgang 1 Bit      */
   UINT8 shader[BUS_DO31_SHADER_SIZE_GET];  /* je Rollladen 2 Bit: 00: nicht konfiguriert */
                                            /*                     01: AUF        */
                                            /*                     10: AB         */
                                            /*                     11: STOP       */
} TBusDevGetStateDo31;
...
typedef struct {
   UINT8  switchState;  /* bitfield for 8 switch states; 0 off, 1 on */        
} TBusDevReqSwitchState;        /* type 0x11 */

typedef struct {  
   UINT8  switchState;   /* same as request */
} TBusDevRespSwitchState;       /* type 0x12 */

typedef struct {        
   UINT8 clientAddr[BUS_MAX_CLIENT_NUM];  /* address 0xff means 'not used' */
